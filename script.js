...
window.onload = () => {
  // Check if the browser Supports ee Dropper
  if ('EyeDropper' in window) {
    pickColor.classList.remove('hide')
    eyeDropper = new EyeDropper()
  } else {
    error.classList.remove('hide')
    error.innerText = 'Your Browser doest support Eyedropper API'
...
const colorSelector = async () => {
  const color = await eyeDropper
    ...
    .catch((err) => {
      error.classList.remove('hide')
      // if user press escape to close the eyedropper
...
// Allow user to choose image of their own choise
fileInput.onchange = () => {
  ...
  reader.onload = () => {
    ...
    // set src attribute image to result/input file
    image.setAttribute('src', reader.result)
  }
}

// Function to copy te color code
